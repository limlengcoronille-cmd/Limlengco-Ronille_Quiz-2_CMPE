    ...
    try: 
        ...
        while True:
            try:
                ...
                print("Invalid numbers. Please enter numbers separated by commas.")
        ...
        if choice == "1":
            ...
            if len(num) < 2:
                print("Sample standard deviation requires at least two numbers.")
            else:
                mean = sum(num) / len(num)
                variance = sum((x - mean) ** 2 for x in num) / (len(num) - 1)
                result = math.sqrt(variance)
                print("Formula: SD = √( Σ(x - mean)² / (n - 1) )")
                steps = " + ".join([f"({x} - {mean})²" for x in num])
                print(f"Solution: √( {steps} / {len(num)-1} )")
                print(f"        = √( {variance} )")
